...
| **Exibição de Placar em Rawtext (`DAY_COUNTER`)** | Rawtext `{"score": {"name": "*", "objective": "dayCounter"}}` | Sincronização contínua de `DAY_COUNTER` para `@a` e substituição de nomes dummy por `*` no rawtext | O parser JSON do Bedrock não resolve scores de fake players como `DAY_COUNTER`, exigindo seletores válidos (`*` ou `@p`). |
...
| **Rawtext Score Display (`DAY_COUNTER`)** | Rawtext `{"score": {"name": "*", "objective": "dayCounter"}}` | Continuous sync from `DAY_COUNTER` to `@a` and dummy name conversion to `*` | Bedrock JSON parser does not resolve arbitrary dummy scoreboard names; requires valid entity selectors (`*` or `@p`). |

...
| **Exibição de Placar em Rawtext (`DAY_COUNTER`)** | Rawtext `{"score": {"name": "DAY_COUNTER", "objective": "dayCounter"}}` | Preservação direta do fake player `DAY_COUNTER` com sincronização contínua para `@a` | No Bedrock, o componente `score` em rawtext suporta nomes literais de fake players. A preservação do nome `DAY_COUNTER` garante a exibição correta e persistente do dia. |
...
| **Rawtext Score Display (`DAY_COUNTER`)** | Rawtext `{"score": {"name": "DAY_COUNTER", "objective": "dayCounter"}}` | Direct preservation of fake player `DAY_COUNTER` with continuous sync to `@a` | In Bedrock, the rawtext `score` component natively supports literal fake player names. Preserving `DAY_COUNTER` guarantees accurate and persistent day display. |
